import type { CompareResult, TableSummary, RowDiff } from "./types";
...
function truncate(val: unknown, maxLen = 40): string {
  const str = val === null ? "NULL" : val === undefined ? "undefined" : String(val);
...
export async function printReport(
  ...
  for (const table of result.tables) {
    ...
    if (table.missingInPostgres > 0) {
      console.log(
        c.yellow(`  Missing in Postgres: ${table.missingInPostgres}`)
      );
    }
...
function printTableDiffs(table: TableSummary, c: any) {
  printDiffs(table.diffs, c);
}

function printDiffs(diffs: RowDiff[], c: any) {
